...
(define hicolor-icon-theme-lutris
  (package
    ...
    (version "0.17-7fea043d")
    (source
     (origin
       (method git-fetch)
       (uri (git-reference
             (url "https://github.com/lutris/lutris-runtime")
             (commit "7fea043dafe2a03c66f72eb3079aa9abea626640")))
       (file-name (git-file-name name version))
       (sha256
        (base32 "1pl57n0xpkk9vxwljbq74j8gbwzy7a787s99hwj4cy6cd1d4kfsx"))))
    ...
    (source
     (origin
       ...
       (uri (string-append
             ...
             ".tar.gz"))
       (sha256
        (base32 "15jhbm9v259xnc6h95z1npivbbw0siyh68s4n0h3jiqp4n4vmval"))))
    (build-system copy-build-system)
    (propagated-inputs (list nss-certs)) ;Otherwise, TLS doesn't work and it's impossible to connect to itch.io, gog, etc.
    (inputs (list
                     ...
                     (lambda* (#:key #:allow-other-keys)
                       (wrap-program (string-append #$output "/bin/lutris")
                         ...
                         `("GSETTINGS_SCHEMA_DIR" =
                           ,(list #$(file-append gsettings-desktop-schemas "/share/glib-2.0/schemas/")
                                   #$(file-append gtk+ "/share/glib-2.0/schemas/")
                                   "$GSETTINGS_SCHEMA_DIR"))
                         `("PATH" =

...
(define lutris-client-libs ; For TLS support, to login into itch.io, gog, etc.
  `(("nss-certs" ,nss-certs)
    ("cairo" ,cairo)
...
(define (lutris-container-for driver)
  (nonguix-container
    (name "lutris")
    (wrap-package lutris-client)
    (run "/bin/lutris")
    (modules '(gtk))
    (sandbox-home ".local/share/lutris-sandbox-home")
    (packages
      (modify-inputs `(,@lutris-client-libs
...
(define-public lutris-for
  (compose nonguix-container->package lutris-container-for))

(define-public lutris-for-nvdb (lutris-for nvdb))
